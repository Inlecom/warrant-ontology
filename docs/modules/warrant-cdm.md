...
| `cdm:affectsComponent` | `cdm:FailureMode` ∪ `cdm:Vulnerability` | `davom:Component` ∪ `davom:System` ∪ `davom:Subsystem` | Links a FailureMode or Vulnerability to the element(s) it degrades, puts out of service, or exposes. Normally a Component and its subclasses (SensorComponent, SoftwareComponent, etc.); a System or Subsystem is admissible where the failure mode is analysed at that level, as in a software system whose data output goes stale. |
...
| `cdm:mayCause` | `cdm:Deviation` ∪ `cdm:ProcessModelFlaw` ∪ `cdm:UnsafeControlAction` | `cdm:Hazard` ∪ `cdm:UnsafeControlAction` ∪ `cdm:ProcessModelFlaw` | Causal propagation within the CDM chain. A deviation or an unsafe input may also cause a ProcessModelFlaw, which is the STPA path by which a wrong or falsified input corrupts the controller's model of the process before any unsafe control action is issued. The link from a Deviation, Hazard, or UnsafeControlAction to an assr:AssuranceDegradation is expressed by assr:causesAssuranceDegradation (declared in warrant-assurance, which imports this module), not by this property. |
